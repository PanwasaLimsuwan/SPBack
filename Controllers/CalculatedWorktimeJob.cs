        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            while (!stoppingToken.IsCancellationRequested)
            {
                try
                {
                    ...
                    // ✅ ใช้ endpoint เดิมได้เลย (แต่ logic ภายในเปลี่ยนเป็น Transactions แล้ว)
                    var response = await client.PostAsync(
                        "http://localhost:5000/api/GateToWorktime", 
                        null
                    );
                    ...
                    if (response.IsSuccessStatusCode)
                    {
                        ...
                        Console.WriteLine($"[CalculatedWorktimeJob] Success @ {DateTime.Now}: {content}");
                    }
                    else
                    {
                        Console.WriteLine($"[CalculatedWorktimeJob] Failed @ {DateTime.Now}: {response.StatusCode}");
                    }
                }
                catch (Exception ex)
                {
